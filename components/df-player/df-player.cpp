...
*/
/**************************************************************************/
bool DFPlayerMini_Fast::begin(uart_port_t uart_num, bool debug,
...
{
    return query(dfplayer::GET_FOLDER_FILES, (folder >> 8) & 0xFF,
                 folder & 0xFF);
}
...
{
    uart_write_bytes(uart_num, &sendStack.start_byte, 1);
    uart_write_bytes(uart_num, &sendStack.version, 1);
    uart_write_bytes(uart_num, &sendStack.length, 1);
    uart_write_bytes(uart_num, &sendStack.commandValue, 1);
    uart_write_bytes(uart_num, &sendStack.feedbackValue, 1);
    uart_write_bytes(uart_num, &sendStack.paramMSB, 1);
    uart_write_bytes(uart_num, &sendStack.paramLSB, 1);
    uart_write_bytes(uart_num, &sendStack.checksumMSB, 1);
    uart_write_bytes(uart_num, &sendStack.checksumLSB, 1);
    uart_write_bytes(uart_num, &sendStack.end_byte, 1);
...
void DFPlayerMini_Fast::flush()
{
    ...
    while (size) {
        uart_read_bytes(uart_num, buf, std::min(size, sizeof(buf)), 10);
        uart_get_buffered_data_len(uart_num, &size);
    }
}
...
bool DFPlayerMini_Fast::parseFeedback()
{
    while (true) {
        ...
        int len = uart_read_bytes(uart_num, &recChar, 1, 100);
        if (len == 0) {
        ...
        switch (state) {
            case find_start_byte: {
                // if (_debug) Serial.println(F("find_start_byte"));
            ...
            case find_ver_byte: {
                // if (_debug) Serial.println(F("find_ver_byte"));

                if (recChar != dfplayer::VER) {
                    // if (_debug) Serial.println(F("ver error"));
            ...
            case find_len_byte: {
                // if (_debug) Serial.println(F("find_len_byte"));

                if (recChar != dfplayer::LEN) {
                    // if (_debug) Serial.println(F("len error"));
            ...
            case find_command_byte: {
                // if (_debug) Serial.println(F("find_command_byte"));
            ...
            case find_feedback_byte: {
                // if (_debug) Serial.println(F("find_feedback_byte"));
            ...
            case find_param_MSB: {
                // if (_debug) Serial.println(F("find_param_MSB"));
            ...
            case find_param_LSB: {
                // if (_debug) Serial.println(F("find_param_LSB"));
            ...
            case find_checksum_MSB: {
                // if (_debug) Serial.println(F("find_checksum_MSB"));
            ...
            case find_checksum_LSB: {
                // if (_debug) Serial.println(F("find_checksum_LSB"));

                recStack.checksumLSB = recChar;

                int recChecksum =
                    (recStack.checksumMSB << 8) | recStack.checksumLSB;
            ...
            case find_end_byte: {
                // if (_debug) Serial.println(F("find_end_byte"));

                if (recChar != dfplayer::EB) {
                    // if (_debug) Serial.println(F("eb error"));
...
void DFPlayerMini_Fast::printError()
{
    if (recStack.commandValue == 0x40) {
        switch (recStack.paramLSB) {
            case 0x1: {
                // Serial.println(
                //     F("Module busy (this info is returned when the "
                //       "initialization is not done)"));
                break;
            }
            case 0x2: {
                // Serial.println(
                //     F("Currently sleep mode(supports only specified device in
                //     "
                //       "sleep mode)"));
                break;
            }
            case 0x3: {
                // Serial.println(
                //     F("Serial receiving error(a frame has not been received "
                //       "completely yet)"));
                break;
            }
            case 0x4: {
            ...
            case 0x5: {
                // Serial.println(
                //     F("Specified track is out of current track scope"));
                break;
            }
            case 0x6: {
            ...
            case 0x7: {
                // Serial.println(
                //     F("Insertion error(an inserting operation only can be
                //     done "
            ...
            case 0x8: {
                // Serial.println(
                //     F("SD card reading failed(SD card pulled out or
                //     damaged)"));
                break;
            }
            case 0xA: {
